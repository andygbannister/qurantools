...
<html>
	<head>
		<?php
            ...
            window_title("Formulaic Diversity Per Sura");
        ?>
...
?>
	...
	(SELECT COUNT(DISTINCT CONCAT(`FORMULA`,'-',`TYPE`)) FROM `FORMULA-LIST` WHERE `START SURA`=`Sura Number` AND `LENGTH`=" . db_quote($FORMULA_LENGTH) . ") formula_count
	...
	$sort_order");
            ...
            {
                $result = db_query("SELECT `Sura Number`, `Provenance`, `Words`,
	(SELECT COUNT(DISTINCT `FORMULA`) FROM `FORMULA-LIST` WHERE `START SURA`=`Sura Number` AND `TYPE`='" . db_quote($FORMULA_TYPE) . "' AND `LENGTH`=" . db_quote($FORMULA_LENGTH) . ") formula_count
	FROM `SURA-DATA` $filter_provenance
	$sort_order");
            }
                            echo "\"data\": [";
...
if ($miniMode)
{
    $expander_link = "../charts/chart-formulaic-diversity-by-sura.php?TYPE=$FORMULA_TYPE&L=$FORMULA_LENGTH";
...
echo "    <h2>";
echo "      <a href='../formulae/list_formulae.php?L=$FORMULA_LENGTH&TYPE=$FORMULA_TYPE' CLASS=linky>Formulaic Diversity per Sura</a>";
echo "    </h2>";
...
if (!$miniMode)
{
    ...
    echo "<form action='chart_formulaic_diversity_per_sura.php?PROV=$PROV&PER100=$per100words&SORT=" . $_GET["SORT"] . "' method=POST>";
    ...
    echo "  	  <a href='chart_formulaic_diversity_per_sura.php?L=$FORMULA_LENGTH&TYPE=$FORMULA_TYPE&PER100=$per100words&SORT=" . $_GET["SORT"] . "' class='" . $all_suras_selected . "'>";
    echo "	  	  All Suras";
    echo "	    </a>";

    echo "	    <a href='chart_formulaic_diversity_per_sura.php?PROV=MECCAN&L=$FORMULA_LENGTH&TYPE=$FORMULA_TYPE&PER100=$per100words&SORT=" . $_GET["SORT"] . "' class='" . $meccan_suras_selected . "'>";
    echo "		    Meccan";
    echo "	    </a>";

    echo "	    <a href='chart_formulaic_diversity_per_sura.php?PROV=MEDINAN&L=$FORMULA_LENGTH&TYPE=$FORMULA_TYPE&PER100=$per100words&SORT=" . $_GET["SORT"] . "' class='" . $medinan_suras_selected . "'>";
    echo "		    Medinan";
    echo "	    </a>";
    ...
    echo "      <a href='chart_formulaic_diversity_per_sura.php?L=$FORMULA_LENGTH&TYPE=$FORMULA_TYPE&PROV=$PROV&SORT=" . $_GET["SORT"] . "' class='$all_occurrences_selected'>";
    echo "        All Occurrences";
    echo "      </a>";
    echo "      <a href='chart_formulaic_diversity_per_sura.php?L=$FORMULA_LENGTH&TYPE=$FORMULA_TYPE&PER100=1&PROV=$PROV&SORT=" . $_GET["SORT"] . "' class='$occurrences_per_100_words_selected'>";
    ...
    echo "        <a href='chart_formulaic_diversity_per_sura.php?L=$FORMULA_LENGTH&TYPE=$FORMULA_TYPE&PER100=$per100words&PROV=$PROV' class='$default_sort_selected'>";
    echo "          Sura Number";
    echo "        </a>";
    echo "        <a href='chart_formulaic_diversity_per_sura.php?L=$FORMULA_LENGTH&TYPE=$FORMULA_TYPE&PER100=$per100words&PROV=$PROV&SORT=1' class='$first_sort_option_selected'>";
    echo "          Number of Different Formulae Used";
    echo "        </a>";

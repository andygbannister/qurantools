...
<head>
		<?php
            ...
            window_title("Chart of Unique Formula Per Sura");
        ?>
    ...
    var revenueChart = new FusionCharts({
        ...
        "dataSource":  {
          "chart": {
            ...
            if (!$per100words)
            {
                echo "\"yAxisName\": \"Number of Different Formulae Used\",";
            }
            else
            {
                echo "\"yAxisName\": \"Number of Different Formulae Used per 100 Words\",";
            }
            ?>
            "theme": "fint",
            "showValues": "0"
       ...
       if ($FORMULA_LENGTH != "ANY")
       {
           $query_length = "AND `LENGTH`=" . db_quote($FORMULA_LENGTH);
       }

       if ($FORMULA_TYPE == "ANY")
       {
           $result = db_query("SELECT `Sura Number`, `Provenance`, `Words`,
(SELECT COUNT(DISTINCT CONCAT(`FORMULA`,'-',`TYPE`)) FROM `FORMULA-LIST` WHERE `START SURA`=`Sura Number` $query_length AND `APPEARS IN HOW MANY SURAS`=1) formula_count
...
$sort_order");
        ...
        {
            $result = db_query("SELECT `Sura Number`, `Provenance`, `Words`,
(SELECT COUNT(DISTINCT `FORMULA`) FROM `FORMULA-LIST` WHERE `START SURA`=`Sura Number` AND `TYPE`='" . db_quote($FORMULA_TYPE) . "' $query_length AND `APPEARS IN HOW MANY SURAS`=1) formula_count
FROM `SURA-DATA` $filter_provenance
$sort_order");
        }
                      echo "\"data\": [";
               ...
               for ($i = 0; $i < db_rowcount($result); $i++)
               {
                   ...
                   if (!$miniMode)
                   {
                       echo "link:\"../formulae/list_formulae.php?SURA=" . $ROW["Sura Number"] . "&L=$FORMULA_LENGTH&TYPE=$FORMULA_TYPE&UNIQUE=1\",";
                   }
...
if ($miniMode)
{
    $expander_link = "/charts/chart_formulae_unique_to_sura.php?TYPE=$FORMULA_TYPE&L=$FORMULA_LENGTH";
...
echo "  <h2>";
echo "     <a href='../formulae/formulaic_density_by_sura.php?L=$FORMULA_LENGTH&TYPE=$FORMULA_TYPE' CLASS=linky>";
echo "      Unique Formula Per Sura";
echo "    </a>";
echo "  </h2>";
...
if (!$miniMode)
{
    ...
    echo "<form action='chart_formulae_unique_to_sura.php?PROV=$PROV&PER100=$per100words&SORT=" . $_GET["SORT"] . "' method=POST>";
    ...
    echo "  	  <a href='chart_formulae_unique_to_sura.php?L=$FORMULA_LENGTH&TYPE=$FORMULA_TYPE&PER100=$per100words&SORT=" . $_GET["SORT"] . "' class='" . $all_suras_selected . "'>";
    echo "	  	  All Suras";
    echo "	    </a>";

    echo "	    <a href='chart_formulae_unique_to_sura.php?PROV=MECCAN&L=$FORMULA_LENGTH&TYPE=$FORMULA_TYPE&PER100=$per100words&SORT=" . $_GET["SORT"] . "' class='" . $meccan_suras_selected . "'>";
    echo "		    Meccan";
    echo "	    </a>";

    echo "	    <a href='chart_formulae_unique_to_sura.php?PROV=MEDINAN&L=$FORMULA_LENGTH&TYPE=$FORMULA_TYPE&PER100=$per100words&SORT=" . $_GET["SORT"] . "' class='" . $medinan_suras_selected . "'>";
    echo "		    Medinan";
    echo "	    </a>";
    ...
    echo "      <a href='chart_formulae_unique_to_sura.php?L=$FORMULA_LENGTH&TYPE=$FORMULA_TYPE&PROV=$PROV&SORT=" . $_GET["SORT"] . "' class='$all_occurrences_selected'>";
    echo "        All Occurrences";
    echo "      </a>";
    echo "      <a href='chart_formulae_unique_to_sura.php?L=$FORMULA_LENGTH&TYPE=$FORMULA_TYPE&PER100=1&PROV=$PROV&SORT=" . $_GET["SORT"] . "' class='$occurrences_per_100_words_selected'>";
    ...
    echo "        <a href='chart_formulae_unique_to_sura.php?L=$FORMULA_LENGTH&TYPE=$FORMULA_TYPE&PER100=$per100words&PROV=$PROV' class='$default_sort_selected'>";
    echo "          Sura Number";
    echo "        </a>";
    echo "        <a href='chart_formulae_unique_to_sura.php?L=$FORMULA_LENGTH&TYPE=$FORMULA_TYPE&PER100=$per100words&PROV=$PROV&SORT=1' class='$first_sort_option_selected'>";
    echo "          Number of Formulae Used";
    echo "        </a>";

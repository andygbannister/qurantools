...
window_title("Chart of Noun Occurrences - Based on $ARABIC (" . htmlentities($ROOT) . ")");
...
<script type="text/javascript">
  FusionCharts.ready(function(){
    var revenueChart = new FusionCharts({
       ...
       $result = db_query("SELECT `Sura Number`, `Provenance`, `Words`, (SELECT COUNT(*) FROM `QURAN-DATA` WHERE `SURA`=`Sura Number` AND BINARY(`QTL-ROOT-TRANSLITERATED`)='" . db_quote($ROOT) . "' AND `TAG EXPLAINED`='NOUN') count_root FROM `SURA-DATA` $filter_prov $sort_order");
       ...
       // if we are showing both Meccan and Medinan suras, we want two datasets and a legend
               ...
               for ($i = 0; $i < db_rowcount($result); $i++)
               {
                   ...
                   if (!$miniMode)
                   {
                       echo "link:\"../verse_browser.php?S=ROOT:" . urlencode($ROOT) . "@[NOUN] RANGE:" . $ROW["Sura Number"] . "\",";
                   }
...
if ($miniMode)
{
    ...
    echo "  <a href='/charts/chart_nouns.php?ROOT=" . urlencode($_GET["ROOT"]) . "'>";
...
echo "  <h2>";
echo "    <a href='../counts/count_nouns.php' class=linky>Noun Occurrences</a> Based On Root: $ARABIC (<span class='override-cabin-font-for-subtitles-with-transcription $user_preference_transliteration_style'>" . htmlentities($ROOT) . "</span>)";
echo "  </h2>";
...
if (!$miniMode)
{
    ...
    echo "  	  <a href='chart_nouns.php?ROOT=" . urlencode($ROOT) . "&COUNT=$COUNT&PROV=ANY&SORT=" . $_GET["SORT"] . "' class='" . $all_suras_selected . "'>";
    echo "	  	  All Suras";
    echo "	    </a>";

    echo "	    <a href='chart_nouns.php?ROOT=" . urlencode($ROOT) . "&COUNT=$COUNT&PROV=MECCAN&SORT=" . $_GET["SORT"] . "' class='" . $meccan_suras_selected . "'>";
    echo "		    Meccan";
    echo "	    </a>";

    echo "	    <a href='chart_nouns.php?ROOT=" . urlencode($ROOT) . "&COUNT=$COUNT&PROV=MEDINAN&SORT=" . $_GET["SORT"] . "' class='" . $medinan_suras_selected . "'>";
    echo "		    Medinan";
    echo "	    </a>";
    ...
    echo "      <a href='chart_nouns.php?ROOT=" . urlencode($ROOT) . "&COUNT=OCC&PROV=$PROV&SORT=" . $_GET["SORT"] . "' class='$all_occurrences_selected'>";
    echo "        All Occurrences";
    echo "      </a>";
    echo "      <a href='chart_nouns.php?ROOT=" . urlencode($ROOT) . "&COUNT=PER100&PROV=$PROV&SORT=" . $_GET["SORT"] . "' class='$occurrences_per_100_words_selected'>";
    ...
    echo "        <a href='chart_nouns.php?ROOT=" . urlencode($ROOT) . "&COUNT=$COUNT&PROV=$PROV' class='$default_sort_selected'>";
    echo "          Sura Number";
    echo "        </a>";
    echo "        <a href='chart_nouns.php?ROOT=" . urlencode($ROOT) . "&COUNT=$COUNT&SORT=1&PROV=$PROV' class='$first_sort_option_selected'>";
    echo "          Occurrences";
    echo "        </a>";
